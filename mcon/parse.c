...
int mongo_store_option(mongo_con_manager *manager, mongo_servers *servers, char *option_name, char *option_value, char **error_message)
{
	...
	if (strcasecmp(option_name, "safe") == 0) {
		mongo_manager_log(manager, MLOG_PARSE, MLOG_INFO, "- Found option 'safe': %s", option_value);
		if (strcmp(option_value, "true") == 0) {
			servers->default_safe = 1;
		} else if (strcmp(option_value, "false") == 0) {
			servers->default_safe = 0;
		} else {
			*error_message = strdup("The value of 'safe' needs to be either 'true' or 'false'");
			return 3;
		}
		return 0;
	}
